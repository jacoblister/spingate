...
invertedDigit = digit.map((dig) => all.reduce((acc, bit) => dig.includes(bit) ? acc : acc.concat(bit), []))

value = [
    [], [A0], [A1], [A0, A1], [A2], [A2, A0], [A2, A1], [A2, A1, A0],
    [A3], [A3, A0], [A3, A1], [A3, A0, A1], [A3, A2], [A3, A2, A0], [A3, A2, A1], [A3, A2, A1, A0]
]
invertedValue = value.map((dig) => all.reduce((acc, bit) => dig.includes(bit) ? acc : acc.concat(bit), []))


clearBits = (bits) => bits.reduce((acc, bit) => acc + clear(bit), "")
segToggle = (digit) => digit.reduce((acc, bit) => acc + toggle(bit), "")
segClearIf = (digit, DP) => digit.reduce((acc, bit) => acc + and(DP, bit), "")
segSetIf = (digit, DP) => digit.reduce((acc, bit) => acc + or(DP, bit), "")
test = (pattern, bits, dst) => set(dst) +
    pattern.reduce((acc, bit) => acc + toggle(bit), "") + 
    bits.reduce((acc, bit) => acc + and(bit, dst), "") +
    pattern.reduce((acc, bit) => acc + toggle(bit), "") + toggle(DP)
convert = (from, to) => test(from, all, DP) + segClearIf(all, DP) + toggle(DP) + segSetIf(to, DP) + clear(DP)
toNum = () => invertedDigit.reduce((acc, pattern, i) => acc + convert(pattern, value[i]), "")
toSeg = () => invertedValue.reduce((acc, pattern, i) => acc + convert(pattern, digit[i]), "")
...
setNum = (i) => toNum() + swapAB() + clearA() + value[i].reduce((acc, bit) => acc + set(bit), "") + toSeg()
